...
import {Like} from '../../modeller/like';
import {Kullanici} from '../../modeller/kullanici';

@Component({
  selector: 'app-sorular-sayfa',
  templateUrl: './sorular-sayfa.component.html',
})
export class SorularSayfaComponent implements OnInit {
      ...
      map(
        changes => {
          return changes.map(c => {
            ...
            this.db.list<Like>('begeni', ref => ref.orderByChild('soruId').equalTo(soru.id))
              .snapshotChanges()
              .subscribe(cevapChanges => {
            ...
            this.db.list<Category>('kategori', ref => ref.orderByKey().equalTo(soru.kategoriId))
              .snapshotChanges()
